...
export const extractVulnerabilitiesFromReportCall = async (
  ...
  const client = await getRestClient();
  const response = await client.request(`api/v1/reports/${reportId}/extract-vulnerabilities`, 'POST');
...
export const editVulnerabilityCall = async (vulnerability: Vulnerability | FormData): Promise<boolean> => {
  ...
  const response = await client.request(`api/v1/vulnerabilities/${vulnerabilityId}`, 'PUT', body);
  return response.data as boolean;
};
// --- USERS ---
...
export const getForumCategories = async (): Promise<ForumCategory[]> => {
  const client = await getRestClient();
  const response = await client.request('api/v1/forum/categories', 'GET');
  return response.data as ForumCategory[];
};

// Get a single forum category by slug
export const getForumCategoryBySlug = async (slug: string): Promise<ForumCategory> => {
  const client = await getRestClient();
  const response = await client.request(`api/v1/forum/categories/${slug}`, 'GET');
  return response.data as ForumCategory;
};

// Get threads for a category with pagination and sorting
export const getForumThreads = async (params: ThreadSearchParams): Promise<PaginatedThreadsResponse> => {
  const client = await getRestClient();
  ...
  const url = `api/v1/forum/threads${queryString ? `?${queryString}` : ''}`;

  const response = await client.request(url, 'GET');
  return response.data as PaginatedThreadsResponse;
};

// Get a single thread by ID with full content
export const getForumThreadById = async (threadId: number): Promise<ForumThreadDetail> => {
  const client = await getRestClient();
  const response = await client.request(`api/v1/forum/threads/${threadId}`, 'GET');
  return response.data as ForumThreadDetail;
};

// Get a single thread by slug with full content
export const getForumThreadBySlug = async (categorySlug: string, threadSlug: string): Promise<ForumThreadDetail> => {
  const client = await getRestClient();
  const response = await client.request(`api/v1/forum/categories/${categorySlug}/threads/${threadSlug}`, 'GET');
  return response.data as ForumThreadDetail;
};

// Create a new thread (requires authentication)
export const createForumThread = async (params: CreateThreadParams): Promise<ForumThread> => {
  const client = await getRestClient();
  const response = await client.request('api/v1/forum/threads', 'POST', params);
