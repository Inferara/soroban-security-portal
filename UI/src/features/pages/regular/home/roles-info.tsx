...
export const RolesInfo: FC<RolesInfoProps> = ({ isCompact = false }) => {
  ...
  const rolesData = [
    ...
    },
    {
      ...
      description: 'Contributor are Pilots (see the handbook) or those who expressed interest in contributing',
    },
    {
      name: 'Moderator',
  ...
  return (
    ...
    >
      ...
      >
        {isAuthenticated ? (
          <>
            ...
            >
              ...
              >
                <Button variant="outlined" size="large" onClick={() => navigate('/profile')} sx={{ px: 3, py: 1 }}>
                  My Profile
                </Button>
              </Box>
            </Box>
          ...
          >
            <Box sx={{ width: '100%', display: 'flex', justifyContent: 'center' }}>
              <Box sx={{ flex: { xs: '0 1 auto', md: 1 }, textAlign: { xs: 'center', md: 'right' }, width: '100%', maxWidth: 640 }}>
                <Typography variant="body1" sx={{ mb: 2 }}>
                  Your contributions help improve the Portal and secure the Soroban ecosystem.
                </Typography>
                <Typography variant="body1">
            ...
            >
              <Button variant="contained" size="large" onClick={() => navigate('/login')} sx={{ px: 3, py: 1 }}>
                Log In
              </Button>
            </Box>
          </Box>
        ...
        >
          <Box sx={{ width: '100%', display: 'flex', justifyContent: 'center' }}>
            <Box sx={{ flex: { xs: '0 1 auto', md: 1 }, textAlign: { xs: 'center', md: 'left' }, width: '100%', maxWidth: 640 }}>
              <Typography variant="body1" sx={{ mb: 2 }}>
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', flexDirection: 'column', pt: { xs: 5, md: 10 } }}>
        <HomepageSubtitle title="Roles breakdown" isLeft={false} isCompact={isCompact} />
        {isCompact ? (
              ...
              <Box component="tbody">
                {rolesData.map((r) => (
                  <Box component="tr" key={r.name}>
                    ...
                    <Box component="td" sx={{ p: 2 }}>
                      {r.permissions.includes('download') ? '✅' : '❌'}
                    </Box>
                    <Box component="td" sx={{ p: 2 }}>
                      {r.permissions.includes('create') ? '✅' : '❌'}
                    </Box>
                    <Box component="td" sx={{ p: 2 }}>
                      {r.permissions.includes('approve') ? '✅' : '❌'}
                    </Box>
                    <Box component="td" sx={{ p: 2, textAlign: 'left', width: '40%' }}>
                      {r.description}
                    </Box>
                  </Box>
                ))}
